...
const { hydrate: hydratePhoto, photos, loading, savePhoto, restorePhoto } = usePhoto()
const { hydrate: hydrateFamily } = useFamily()
const timelineMode = ref(false)
const galleryItems = computed(() => photos.value.map((item) => ({ id: item.id, url: item.restoredUrl || item.imageUrl, caption: item.caption, year: item.year, meta: `${item.location} · ${item.isRestored ? '已修复' : '原片'}`, peopleIds: item.people })))
const timelineItems = computed(() => photos.value.map((item) => ({ id: item.id, title: item.caption, description: item.location, date: `${item.year}-01-01`, peopleIds: item.people })))
...
async function hydrate() {
  await Promise.all([hydratePhoto(), hydrateFamily()])
}
